    ...
    public void paintGraph(Graphics g) {
      for (Vertex v : dijkstra.getVertices()) {
        for (Edge edge : v.neighbors) {
          paintEdge(g, edge.source, edge.destination, edge.weight, Color.LIGHT_GRAY, DEFAULT_THICKNESS, 255);
        }
      }
      for (Vertex v : dijkstra.getVertices()) {
        paintVertex(g, v);
      ...
      for (String overlayType : overlayEdges.keySet()) {
        if (overlayType.equals("unweighted")) {
          for (Edge edge : overlayEdges.get(overlayType)) {
            paintEdge(g, edge.source, edge.destination, edge.weight, Color.RED, 8, 50);
          }
        }
        if (overlayType.equals("weighted")) {
          for (Edge edge : overlayEdges.get(overlayType)) {
            paintEdge(g, edge.source, edge.destination, edge.weight, Color.GREEN, 8, 50);
          }
        }
